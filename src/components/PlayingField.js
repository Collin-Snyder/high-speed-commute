...
const PlayingField = ({ playerCar, bossCar, office, layout }) => {
  if (playerCar === office) {
      return (
        <div className="playingField win">
            <h1>Whew! You're not late. You get a promotion!</h1>
            <button>Ready for tomorrow?</button>
        </div>
      );
    ...
    return (
        <div className="playingField loss">
            ...
            <button>Get a new job and try again?</button>
        </div>
      );
  } else {
    return (
      <div className="playingField">
        ...
        {layout.map((square, index) => (
          <GridSquare squareInfo={square} playerCar={playerCar} bossCar={bossCar} key={index} />
        ))}
      </div>
    );
  }
